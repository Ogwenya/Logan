...
parse_asset() {
    ...
    url=$(echo "$asset_block" | grep "browser_download_url" | sed 's/.*"browser_download_url"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/' | head -n 1)

    digest=$(echo "$asset_block" | grep "digest" | sed 's/.*"sha256:\([^"]*\)".*/\1/' | head -n 1)
...
install_logan() {
    ...
    if [ -z "$asset_info" ]; then
        ...
        echo "$release_json" | grep -o '"name"[[:space:]]*:[[:space:]]*"logan-[^"]*"' | sed 's/.*"logan-\([^"]*\)".*/  - \1/' | sed 's/\.tar\.gz$//' | sed 's/\.zip$//' >&2
        exit 1
    fi
    ...
    if [ -f "logan" ] || [ -f "logan.exe" ]; then
        ...
        if [ -w "$INSTALL_DIR" ]; then
            install -m 755 "$binary_name" "$INSTALL_DIR/logan" || {
                echo "Error: Installation failed" >&2
                rm -rf "$tmp_dir"
                exit 1
            }
        else
            sudo install -sm 755 "$binary_name" "$INSTALL_DIR/logan" || {
                echo "Error: Installation failed" >&2
                echo "Make sure you have sudo privileges or set LOGAN_INSTALL to a writable directory" >&2
                rm -rf "$tmp_dir"
                exit 1
            }
        fi
        ...
        echo ""
        echo "✓ Logan CLI was installed successfully to $INSTALL_DIR/logan"
        echo ""
...
Environment Variables:
  LOGAN_INSTALL    Installation directory (default: /usr/local/bin)
...
Examples:
  ...
  curl -fsSL <installer-url> | sh -s v1.0.0

  # Install to custom location
  curl -fsSL <installer-url> | LOGAN_INSTALL=~/.local/bin sh
...
main() {
    ...
    install_logan "$version" "$platform"
}
